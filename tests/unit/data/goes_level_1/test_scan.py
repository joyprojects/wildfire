...
def test_scan(l1_all_bands_wildfire):
    actual = goes_level_1.GoesScan(bands=l1_all_bands_wildfire)
...
def test_scan_init_bad_args(l1_all_bands_wildfire):
    too_many_bands = l1_all_bands_wildfire + [l1_all_bands_wildfire[0]]
    with pytest.raises(ValueError) as error_message:
        goes_level_1.GoesScan(bands=too_many_bands)
        assert "Too many bands" in error_message

    too_few_bands = l1_all_bands_wildfire[:15]
    with pytest.raises(ValueError) as error_message:
        goes_level_1.GoesScan(bands=too_few_bands)
        assert "Missing bands" in error_message

    l1_all_bands_wildfire[15].dataset.attrs[
        "dataset_name"
    ] = "OR_ABI-L1b-RadM1-M6C01_G16_s20193002000275_e20193002000332_c20193002000379.nc"
    with pytest.raises(ValueError) as error_message:
        goes_level_1.GoesScan(bands=l1_all_bands_wildfire)
        assert "must have same" in error_message


def test_rescale_to_2km(l1_all_bands_wildfire):
    original = goes_level_1.GoesScan(bands=l1_all_bands_wildfire)
...
def test_read_netcdfs(l1_wildfire_scan_filepaths):
    actual = goes_level_1.read_netcdfs(local_filepaths=l1_wildfire_scan_filepaths)
    assert isinstance(actual, goes_level_1.GoesScan)


def test_get_goes_scan_local(l1_wildfire_scan_filepaths):
    local_filepath = l1_wildfire_scan_filepaths
